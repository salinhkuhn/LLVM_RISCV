...
instance : DialectDenote (LLVMPlusRiscV) where
  ...
  | .builtin.unrealized_conversion_cast.LLVMToriscv w, elemToCast, _  => builtin.unrealized_conversion_cast.LLVMToriscv (elemToCast.getN 0 (by simp [DialectSignature.sig, signature]))
...
^bb0(%X : i4, %Y : i4, %Z : i4) :
  ...
  %res1 = add  %x1, %y1 : !i64
  %z1= "builtin.unrealized_conversion_cast.riscvToLLVM" (%res1) : (!i64) -> (i64)
  llvm.return %z1 : i64
}]
