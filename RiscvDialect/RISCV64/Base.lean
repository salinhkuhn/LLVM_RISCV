...
And operations out of selected RISC-V extension:
    - `M`: stadnart integer division and multiplication instruction extension
...
| czero.eqz -- returns zero, if the condition rs2 is equal to zero, else rs1.
| czero.nez -- returns zero, if the condition rs2 is non-zero, else rs1.

/-
TO DO zbb_rytpe full -> will be implementeed as soon as MVP exists.
| andn
| orn
| xnor
| max
...
instance : DialectDenote (RV64) where
  ...
  |.mulu, regs, _ => MUL_pure64_fff_bv (regs.getN 1 (by simp [DialectSignature.sig, signature]))  (regs.getN 0 (by simp [DialectSignature.sig, signature])) -- double check
  ...
  |.mul ,regs, _ => MUL_pure64_ftt_bv (regs.getN 1 (by simp [DialectSignature.sig, signature]))  (regs.getN 0 (by simp [DialectSignature.sig, signature]))
